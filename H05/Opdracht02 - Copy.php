...
function check_credentials($mailadres, $password) {
    global $users;
    foreach ($users as $key => $value){
        if($key == $mailadres AND $value == $password){
            //als er een match is word true aan gegevan anders false
            return true;
        }
    }
...
if(check_credentials($mailadres, $password, $users));{
        exit("welkom!");
    } else {
        exit("u heeft geen toegang");
    }
